...
public class PurchaseQuotationPdf extends AbstractPdfView
{
	...
	public void buildPdfDocument(Map<String, Object> model,
		...
		final String path = System.getProperty("user.dir");  
		 PdfWriter.getInstance(doc, new FileOutputStream(path+"/Purchase Quotation.pdf"));   
	
	        doc.open();	
		PurchaseQuotation purchaseQuotation=(PurchaseQuotation) model.get("viewquotation");
		...
		//cell3.setPadding(3);
		cell3.setPhrase(new Phrase("PURCHASE QUOTATION",font1));
		//cell3.setColspan(1);
		...
		doc.add(tableHD);
		 ...
		 Paragraph p2 = new Paragraph(cid.getCompDesc()+"\n\n"+cid.getCompAddress()+"\n\nContact No :  "+cid.getContactNo()+"\n\nEmail :  "+cid.getEmail()+"\n"+" ", new
		        ...
		        doc.addTitle("Purchase Quotation PDF");
		        doc.add(tblHeaderIn);
	...
	f1.setColor(BaseColor.BLACK);
	    ...
	    celld1 = new PdfPCell(new Phrase("Vender Name      :"+"   "+purchaseQuotation.getTxtCustName(),f1));
	    celld1.setColspan(3);	    
	    tabled.addCell(celld1);
	    
	    celld2 = new PdfPCell(new Phrase("Billing Address  :"+"   "+purchaseQuotation.getTxtBillingTo(),f1));
	    celld2.setColspan(3);
	   ...
	   // doc.add(tabled);
	    ...
	    PdfPCell celld11,celld13;
	    
	    celld11 = new PdfPCell(new Phrase("Quotation No :"+"       "+purchaseQuotation.getTxyQuotationNo(),f1));
	   ...
	   /* PdfPTable tableBlf1 = new PdfPTable(1);
		...
		doc.add(tableBlf1);*/
			...
			tableDetails.setWidths(new float[] {1.5f, 3.0f, 3.5f, 1.8f,2.2f, 2.5f});
			...
			List<purchaseDetails> ordProductList=new ArrayList<purchaseDetails>();		
			ordProductList.addAll(purchaseQuotation.getProductList());
			//int flag=0;
			for(purchaseDetails productList : ordProductList)
			{
				
				PdfPCell cell20 = new PdfPCell();
				cell20.setPhrase(new Phrase(Convert.toString(productList.getSrNo()), font11));
				...
				cell22.setPhrase(new Phrase(Convert.toString(productList.getUnitPrice()), font11));
				cell22.setHorizontalAlignment(Element.ALIGN_RIGHT);
				tableDetails.addCell(cell22);
		
				
				PdfPCell cell25 = new PdfPCell();
				cell25.setPhrase(new Phrase(Convert.toString(productList.getItemTotal()), font11));
			...
			List<TaxDetailsList> qutTaxList=new ArrayList<TaxDetailsList>();		
			qutTaxList.addAll(purchaseQuotation.getTaxList());		
			for(TaxDetailsList tList : qutTaxList)  
			{
				PdfPCell cell20 = new PdfPCell();
			...
			List<LinkedTaxDetails> qutLinkedTaxList=new ArrayList<LinkedTaxDetails>();		
			qutLinkedTaxList.addAll(purchaseQuotation.getLinkedTaxList());	
			for(LinkedTaxDetails lTList : qutLinkedTaxList)
			{
				PdfPCell cell20 = new PdfPCell();
				...
				tableLinkedTaxDetails.addCell(cell21);
					 ...
					 PdfPTable table = new PdfPTable(2);
				        ...
				        PdfPCell stCell=new PdfPCell(new Phrase(Convert.toString(purchaseQuotation.getTxtSubTotal()),font));
				        stCell.setHorizontalAlignment(Element.ALIGN_RIGHT);
				        table.addCell(stCell);
				       // table.addCell(new Phrase());
				        table.addCell(new Phrase("Discount Type",font));
				        String rd=Convert.toString(purchaseQuotation.getRadio());
				        if(rd.equals("per")){
				       ...
				       // table.addCell(celln);
				        ...
				        PdfPCell serCell = new PdfPCell(new Phrase(new Phrase(Convert.toString(purchaseQuotation.getTxtDiscount()),font)));
				        ...
				        PdfPCell disCell =new PdfPCell(new Phrase(Convert.toString(purchaseQuotation.getTxtDisTotal()),font));
				        ...
				        PdfPCell gtCell =new PdfPCell(new Phrase(Convert.toString(purchaseQuotation.getTxtAmtPayble()),font));
			...
			PdfPTable tableTaxDetails = new PdfPTable(2);
			tableTaxDetails.setWidthPercentage(100.0f);		
			tableTaxDetails.setWidths(new float[] {1.8f, 1.2f});		
			
			
			List<TaxDetailsList> ordTaxList=new ArrayList<TaxDetailsList>();		
			ordTaxList.addAll(purchaseQuotation.getTaxList());		
			...
			PdfPTable tableLinkedTaxDetails = new PdfPTable(2);
			tableLinkedTaxDetails.setWidthPercentage(100.0f);		
			tableLinkedTaxDetails.setWidths(new float[] {1.8f, 1.2f});
			
			
			List<LinkedTaxDetails> ordLinkedTaxList=new ArrayList<LinkedTaxDetails>();		
			ordLinkedTaxList.addAll(purchaseQuotation.getLinkedTaxList());	
			for(LinkedTaxDetails lTList : ordLinkedTaxList)
			{
			 ...
			 PdfPTable table = new PdfPTable(3);
		        ...
		        table.setWidths(new float[]{5.0f, 1.8f, 1.2f});
		        
		        table.addCell(new Phrase("VAT TIN No  :-  "+cid.getVatNo(),font));
		        table.addCell(new Phrase("Sub Total",font));
		        PdfPCell stCell=new PdfPCell(new Phrase(Convert.toString(purchaseQuotation.getTxtSubTotal()),font));
		        stCell.setHorizontalAlignment(Element.ALIGN_RIGHT);
		        table.addCell(stCell);
		        table.addCell(new Phrase("CST TIN No   :-  "+cid.getCstNo(),font));
		        table.addCell(new Phrase("Discount Type",font));
		        String rd=Convert.toString(purchaseQuotation.getRadio());
		        if(rd.equals("per")){
			        PdfPCell vtCell=new PdfPCell(new Phrase("Percentage(%)",font));			       
		        ...
		        PdfPCell celln = new PdfPCell(new Phrase("     "+"\""+cid.getCompTerms()+"\"",font));
		        //celln.setHorizontalAlignment(Element.ALIGN_JUSTIFIED_ALL);
		        celln.setRowspan(7);	
		       // celln.setBorderWidthBottom(0.5f);
		        table.addCell(celln);
		      ...
		      //  celln13.setBorderWidthBottom(0.5f);
		       ...
		       doc.add(tablel);
						...
						tablePN.setSpacingBefore(10);
							...
							cellPS.setPhrase(new Phrase(purchaseQuotation.getTxtPubNotes(),font11));
							cellPS.setBorder(Rectangle.NO_BORDER);
							tablePN.addCell(cellPS);
		...
		tableDetails.setWidthPercentage(100.0f);
		tableDetails.setHeaderRows(1);
		tableDetails.setWidths(new float[] {1.5f, 3.0f, 3.5f, 1.8f,1.8f,2.5f, 2.5f});
		tableDetails.setSpacingBefore(10);
		...
		cell1.setPhrase(new Phrase("Unit Price(INR)", font11));
		tableDetails.addCell(cell1);		
		
		cell1.setPhrase(new Phrase("Total(INR)", font11));
		tableDetails.addCell(cell1);
		...
		estTaxList.addAll(purchaseQuotation.getTaxList());		
		...
		estLinkedTaxList.addAll(purchaseQuotation.getLinkedTaxList());	
       ...
       // table.addCell(new Phrase());
        ...
        String rd=Convert.toString(purchaseQuotation.getRadio());
        if(rd.equals("per")){
	        PdfPCell vtCell=new PdfPCell(new Phrase("@"+purchaseQuotation.getTxtDiscount() +"%",font));			       
	        vtCell.setHorizontalAlignment(Element.ALIGN_RIGHT);
	        table.addCell(vtCell);
        }
        else
        {
        	PdfPCell vtCell=new PdfPCell(new Phrase(Convert.toString(purchaseQuotation.getTxtDiscount()),font));			       
	        vtCell.setHorizontalAlignment(Element.ALIGN_RIGHT);
	        table.addCell(vtCell);
        }*/
			      ...
			      //  celln13.setBorderWidthBottom(0.5f);
			       ...
			       /* 
					...
					tablePN.setSpacingBefore(10);
						...
						cellPS.setPhrase(new Phrase(salesEstimate.getTxtpubNotes(),font));
						cellPS.setBorder(Rectangle.NO_BORDER);
						tablePN.addCell(cellPS);
