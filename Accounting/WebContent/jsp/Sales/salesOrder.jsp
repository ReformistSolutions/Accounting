...
{  
     ...
     else if(document.frmsalesorder.txtCName.value=="0")
    {
     ...
     else if(document.frmsalesorder.txtES.value=="")
     {
       alert(" Education Service tax field should not be blank");
       document.frmsalesorder.txtES.focus();
       return false;
     }
     else if(document.frmsalesorder.txtSEC.value=="")
     {
       alert("Secondary education Service tax field should not be blank");
       document.frmsalesorder.txtSEC.focus();
       return false;
     }
...
{	
	...
	if(taxRateCnt==0)
		...
		document.getElementById("txtTaxTotal").value=(parseFloat(document.getElementById("txtTaxTotal").value)+parseFloat(document.getElementById("taxList["+sr+"].txtprevLinkTax").value)).toFixed(2);
		}
		...
		{			
		 ...
		 for(var k=1;k<=length;k++)
			 ...
			 var taxType = document.getElementById("taxList["+k+"].taxType");
				...
				var taxTotal=document.getElementById("txtTaxTotal").value;
				alert("taxTotal 1:"+taxTotal);
				var index=taxTotal.indexOf(".");
				alert("index 2:"+index);
				var froundOfAmt=taxTotal.substring(index+1,taxTotal.length);
				alert("froundOfAmt 3:"+froundOfAmt);
				var taxTotalRound=taxTotal.substring(0,index);
				alert("taxTotalRound 4:"+taxTotalRound);
				var finalroundOfAmt=(froundOfAmt/100).toFixed(2);
				alert("finalroundOfAmt 5:"+finalroundOfAmt);
				var finalRound=1-finalroundOfAmt;
				alert("finalRound 6:"+finalRound);
				 if(froundOfAmt>50)
					{
		...
		{			
		 ...
		 for(var k=1;k<=length;k++)
			 ...
			 var taxType = document.getElementById("taxList["+k+"].taxType");
				...
				var preLTax=document.getElementById("taxList["+k+"].txtprevLinkTax").value;
				alert("preLTax :"+preLTax);
				document.getElementById("taxList["+k+"].txtprevLinkTax").value="0.0";
				document.getElementById("txtTaxTotal").value=(parseFloat(document.getElementById("txtTaxTotal").value)-parseFloat(preLTax)).toFixed(2);

				var taxRateCnt = document.getElementById("linkTCnt"+k).value;
				alert("taxRateCnt :"+taxRateCnt);
				if(taxRateCnt==0)
					{		
					   document.getElementById("getTax"+k).innerHTML="";		 	
					}				
...
<br>
	...
	<div class="panel panel-default"> 
		...
		<div class="col-md-12">
							<div class="panel panel-default">
							<div class="panel-body">
							<form:form action="addSalesOrder.do" method="post" cssClass="form-inline" commandName="salesOrder" name="frmsalesorder" onsubmit="return validateForm()">
							<div class="row">
	...
	<div class="table-responsive">
						...
						<div>
										...
										&nbsp;&nbsp;&nbsp;&nbsp;
										<label>Charges:	</label>																		
										<form:input path="txtTransCost" id="txtTransCost" cssClass="form-control" cssStyle="width:180px;" onkeypress="return isNumberKey(this)" onblur="calculateCharges();checkTaxes();"></form:input>
...
<c:if test="${message==0}">                     
	<script>
		alert("Order Not Added Succesfully.");
	</script>
</c:if>								
</div>
</div>
